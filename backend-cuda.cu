        ...
        __global__ void enforce_velocity_boundaries_kernel(
            ...
            if (x <= nx && y < ny && z < nz) {
                ...
                if (x == 0) {
                    ...
                    else if (bx_min == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_x[u_index] = fminf(velocity_x[index_3d(1, y, z, nx + 1, ny)], 0.0f);
                    else velocity_x[u_index] = 0.0f;
                }
                if (x == nx) {
                    if (bx_max == VISUAL_SMOKE_BOUNDARY_INFLOW) velocity_x[u_index] = inflow_velocity_x_max;
                    else if (bx_max == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_x[u_index] = fmaxf(velocity_x[index_3d(nx - 1, y, z, nx + 1, ny)], 0.0f);
                    else velocity_x[u_index] = 0.0f;
                }
            }
            ...
            if (x < nx && y <= ny && z < nz) {
                ...
                if (y == 0) {
                    ...
                    else if (by_min == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_y[v_index] = fminf(velocity_y[index_3d(x, 1, z, nx, ny + 1)], 0.0f);
                    else velocity_y[v_index] = 0.0f;
                }
                if (y == ny) {
                    if (by_max == VISUAL_SMOKE_BOUNDARY_INFLOW) velocity_y[v_index] = inflow_velocity_y_max;
                    else if (by_max == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_y[v_index] = fmaxf(velocity_y[index_3d(x, ny - 1, z, nx, ny + 1)], 0.0f);
                    else velocity_y[v_index] = 0.0f;
                }
            }
            ...
            if (x < nx && y < ny && z <= nz) {
                ...
                if (z == 0) {
                    ...
                    else if (bz_min == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_z[w_index] = fminf(velocity_z[index_3d(x, y, 1, nx, ny)], 0.0f);
                    else velocity_z[w_index] = 0.0f;
                }
                if (z == nz) {
                    if (bz_max == VISUAL_SMOKE_BOUNDARY_INFLOW) velocity_z[w_index] = inflow_velocity_z_max;
                    else if (bz_max == VISUAL_SMOKE_BOUNDARY_OUTFLOW) velocity_z[w_index] = fmaxf(velocity_z[index_3d(x, y, nz - 1, nx, ny)], 0.0f);
                    else velocity_z[w_index] = 0.0f;
                }
            }
        }
