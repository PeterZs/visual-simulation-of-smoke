...
int main() {
    ...
    auto add_source_host = [&](std::vector<float>& density, std::vector<float>& temperature, std::vector<float>& velocity_x, std::vector<float>& velocity_y, std::vector<float>& velocity_z) {
    ...
    for (int frame = 0; frame < frames; ++frame) {
        add_source_host(parallel_density, parallel_temperature, parallel_velocity_x, parallel_velocity_y, parallel_velocity_z);
